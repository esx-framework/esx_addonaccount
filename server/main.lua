    ...
    if resourceName == GetCurrentResourceName() then
        ...
        for i = 1, #accounts do
            ...
            if account.shared == 0 then
                ...
                Accounts[account.name][#Accounts[account.name] + 1] =
                    CreateAddonAccount(account.name, account.owner,
                                       account.money)
            else
                if account.money then
                    SharedAccounts[account.name] = CreateAddonAccount(
                                                       account.name, nil,
                                                       account.money)
                else
        ...
        if next(newAccounts) then
            ...
            for i = 1, #newAccounts do
                ...
                SharedAccounts[newAccount[1]] = CreateAddonAccount(
                                                    newAccount[1], nil, 0)
            end
            GlobalState.SharedAccounts = SharedAccounts
        end
...
function GetAccount(name, owner)
    for i = 1, #Accounts[name], 1 do
        if Accounts[name][i].owner == owner then return Accounts[name][i] end
    end
end

function GetSharedAccount(name) return SharedAccounts[name] end

AddEventHandler('esx_addonaccount:getAccount',
                function(name, owner, cb) cb(GetAccount(name, owner)) end)

AddEventHandler('esx_addonaccount:getSharedAccount',
                function(name, cb) cb(GetSharedAccount(name)) end)
